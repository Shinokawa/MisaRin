...
@group(0) @binding(0) var layer_tex: texture_storage_2d<rgba8unorm, read_write>;
@group(0) @binding(1) var layers_tex: texture_2d_array<f32>;
@group(0) @binding(2) var mask_a: texture_storage_2d<rgba8unorm, read_write>;
@group(0) @binding(3) var mask_b: texture_storage_2d<rgba8unorm, read_write>;
...
@group(0) @binding(8) var<storage, read_write> labels: array<atomic<u32>>;
...
fn layers_load(coord: vec2<i32>, layer: i32) -> u32 {
  return unpack_u32(textureLoad(layers_tex, coord, layer, 0));
}

fn mask_a_load(coord: vec2<i32>) -> u32 {
  return unpack_u32(textureLoad(mask_a, coord));
}

fn mask_a_store(coord: vec2<i32>, value: u32) {
  textureStore(mask_a, coord, pack_u32(value));
}

fn mask_b_load(coord: vec2<i32>) -> u32 {
  return unpack_u32(textureLoad(mask_b, coord));
}

fn mask_b_store(coord: vec2<i32>, value: u32) {
  textureStore(mask_b, coord, pack_u32(value));
}
...
const INVALID_LABEL: u32 = 0xffffffffu;
...
const MODE_LABEL_INIT: u32 = 28u;
const MODE_LABEL_HOOK: u32 = 29u;
const MODE_LABEL_COMPRESS: u32 = 30u;
const MODE_LABEL_SEED: u32 = 31u;
const MODE_LABEL_MARK: u32 = 32u;
...
fn sample_color(x: u32, y: u32) -> u32 {
  if (cfg.sample_all_layers == 0u) {
    return layers_load(vec2<i32>(i32(x), i32(y)), i32(cfg.layer_index));
  }
  ...
  for (var i: u32 = 0u; i < count; i = i + 1u) {
    ...
    let src = layers_load(vec2<i32>(i32(x), i32(y)), i32(i));
    let src_a = (src >> 24u) & 0xffu;
    if (src_a == 0u) {
      if (!clipping) {
        mask_alpha = 0u;
...
fn neighbor_has_frontier(x: u32, y: u32) -> bool {
  if (x > 0u) {
    let b = mask_b_load(vec2<i32>(i32(x - 1u), i32(y)));
  ...
  if (y > 0u) {
    let b = mask_b_load(vec2<i32>(i32(x), i32(y - 1u)));
    if ((b & MASK_FRONTIER) != 0u) {
      return true;
    }
  }
...
fn neighbor_has_fill(x: u32, y: u32) -> bool {
  if (x > 0u) {
    let a = mask_a_load(vec2<i32>(i32(x - 1u), i32(y)));
    if ((a & MASK_FILL) != 0u) {
      return true;
    }
  }
  ...
  if (y > 0u) {
    let a = mask_a_load(vec2<i32>(i32(x), i32(y - 1u)));
    if ((a & MASK_FILL) != 0u) {
      return true;
    }
  }
...
fn bucket_fill_main(
  ...
  if (mode == MODE_CLEAR) {
    var a = mask_a_load(vec2<i32>(i32(x), i32(y)));
    if (cfg.selection_enabled == 0u) {
      a = 0u;
    } else {
      a = a & MASK_SELECTION;
    }
    mask_a_store(vec2<i32>(i32(x), i32(y)), a);
    mask_b_store(vec2<i32>(i32(x), i32(y)), 0u);
    return;
  }
  ...
  if (mode == MODE_INIT_INVERSE) {
    let a = mask_a_load(vec2<i32>(i32(x), i32(y)));
    let is_target = (a & MASK_TARGET) != 0u;
    var b: u32 = 0u;
    if (!is_target) {
      b = b | MASK_OPENED;
    }
    mask_b_store(vec2<i32>(i32(x), i32(y)), b);
    return;
  }
  ...
  if (mode == MODE_DILATE_STEP) {
    ...
    for (var dy: i32 = -1; dy <= 1; dy = dy + 1) {
      for (var dx: i32 = -1; dx <= 1; dx = dx + 1) {
        ...
        if (nx < 0 || ny < 0 || nx >= i32(cfg.width) || ny >= i32(cfg.height)) {
          ...
          let b = mask_b_load(vec2<i32>(nx, ny));
          if ((b & input_bit) != 0u) {
            any = true;
          }
        }
    ...
    var b = mask_b_load(vec2<i32>(i32(x), i32(y)));
    if (any) {
      b = b | output_bit;
    } else {
      b = b & ~output_bit;
  ...
  if (mode == MODE_INIT_OUTSIDE) {
    let b_in = mask_b_load(vec2<i32>(i32(x), i32(y)));
  ...
  if (mode == MODE_LABEL_INIT) {
    let index = y * cfg.width + x;
    var a = mask_a_load(vec2<i32>(i32(x), i32(y)));
    let is_target = (a & MASK_TARGET) != 0u;
    var fillable = is_target;
    if (cfg.aux0 != 0u) {
      let b = mask_b_load(vec2<i32>(i32(x), i32(y)));
      if ((b & MASK_OUTSIDE) != 0u) {
        fillable = false;
      }
    }
    let label = select(INVALID_LABEL, index, fillable);
    atomicStore(&labels[index], label);
    a = a & ~MASK_FILL;
    mask_a_store(vec2<i32>(i32(x), i32(y)), a);
    return;
  }

  if (mode == MODE_LABEL_HOOK) {
    let index = y * cfg.width + x;
    ...
    if (x > 0u) {
      let nindex = index - 1u;
      let nlabel = atomicLoad(&labels[nindex]);
      if (nlabel < total && nlabel != label) {
        let hi = max(label, nlabel);
        let lo = min(label, nlabel);
        atomicMin(&labels[hi], lo);
      }
    }
    if (x + 1u < cfg.width) {
      let nindex = index + 1u;
  ...
  if (mode == MODE_LABEL_COMPRESS) {
    ...
    let parent = label;
    let grand = atomicLoad(&labels[parent]);
    if (grand < total && grand != parent) {
      atomicStore(&labels[index], grand);
    }
    return;
  }

  if (mode == MODE_LABEL_SEED) {
    let index = y * cfg.width + x;
    if (index != cfg.start_index) {
      return;
    }
    let total = total_pixels();
    let label = atomicLoad(&labels[index]);
    let root = select(label, INVALID_LABEL, label >= total);
    atomicStore(&state.seed_root, root);
    return;
  }

  if (mode == MODE_LABEL_MARK) {
    let total = total_pixels();
    let seed = atomicLoad(&state.seed_root);
    if (seed >= total) {
      return;
    }
    let index = y * cfg.width + x;
  ...
  if (mode == MODE_APPLY_FILL) {
    ...
    let current = layer_load(vec2<i32>(i32(x), i32(y)));
    if (current != cfg.fill_color) {
      layer_store(vec2<i32>(i32(x), i32(y)), cfg.fill_color);
  ...
  if (mode == MODE_SWALLOW_SEED) {
    ...
    b = b & ~(MASK_FRONTIER | MASK_NEXT | MASK_VISITED);
    if (selection_ok(a)) {
      let color = layer_load(vec2<i32>(i32(x), i32(y)));
    ...
    if (selection_ok(a)) {
      for (var dy: i32 = -1; dy <= 1; dy = dy + 1) {
        for (var dx: i32 = -1; dx <= 1; dx = dx + 1) {
          ...
          let na = mask_a_load(vec2<i32>(nx, ny));
          if ((na & MASK_FILL) != 0u) {
            expanded = true;
          }
        }
  ...
  if (mode == MODE_AA_PASS) {
    ...
    let src = select(
      mask_a_load(vec2<i32>(i32(x), i32(y))),
      layer_load(vec2<i32>(i32(x), i32(y))),
      use_layer_src
    );
    var out = src;
    if (expanded && cfg.aa_factor > 0.0) {
      ...
      for (var dy: i32 = -1; dy <= 1; dy = dy + 1) {
        for (var dx: i32 = -1; dx <= 1; dx = dx + 1) {
          ...
          let neighbor = select(
            mask_a_load(vec2<i32>(nx, ny)),
            layer_load(vec2<i32>(nx, ny)),
            use_layer_src
          );
    ...
    if (use_layer_src) {
      mask_a_store(vec2<i32>(i32(x), i32(y)), out);
    } else {
  ...
  if (mode == MODE_COPY_TEMP_TO_LAYER) {
    let src = mask_a_load(vec2<i32>(i32(x), i32(y)));
    layer_store(vec2<i32>(i32(x), i32(y)), src);
    return;
  }
