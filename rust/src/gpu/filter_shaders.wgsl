...
@group(0) @binding(1)
var dst_tex: texture_storage_2d<r32uint, read_write>;
...
fn color_filter(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  if (filter_type == 0u) {
    ...
    if (a < threshold) {
      ...
      g = 0.0;
      b = 0.0;
      textureStore(dst_tex, vec2<i32>(i32(x), i32(y)), vec4<u32>(pack_argb(0.0, r, g, b), 0u, 0u, 0u));
      return;
    }
...
fn blur_pass(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  if (radius <= 0) {
    let src = textureLoad(src_tex, vec2<i32>(i32(x), i32(y))).x;
    textureStore(dst_tex, vec2<i32>(i32(x), i32(y)), vec4<u32>(src, 0u, 0u, 0u));
    return;
  }
  ...
  for (var i: i32 = -radius; i <= radius; i = i + 1) {
    ...
    let c = textureLoad(src_tex, vec2<i32>(cx, cy)).x;
    let a = unpack_a(c);
  ...
  let out_b = sum_b * inv;
  let out_a = sum_a * inv;
  textureStore(dst_tex, vec2<i32>(i32(x), i32(y)), vec4<u32>(pack_argb(out_a, out_r, out_g, out_b), 0u, 0u, 0u));
}
...
fn morphology_pass(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  if (best_cov <= EPS) {
    if (preserve_alpha) {
      ...
      textureStore(dst_tex, vec2<i32>(i32(x), i32(y)), vec4<u32>(0u, 0u, 0u, 0u));
    }
    return;
  }
  ...
  let g = unpack_g(best);
  let b = unpack_b(best);
  textureStore(dst_tex, vec2<i32>(i32(x), i32(y)), vec4<u32>(pack_argb(best_cov, r, g, b), 0u, 0u, 0u));
}
...
fn antialias_alpha(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  for (var i: u32 = 0u; i < 8u; i = i + 1u) {
    ...
    let n = textureLoad(src_tex, vec2<i32>(nx, ny)).x;
    let na = unpack_a(n);
  ...
  textureStore(dst_tex, vec2<i32>(i32(x), i32(y)), vec4<u32>(pack_argb(new_alpha, out_r, out_g, out_b), 0u, 0u, 0u));
}
...
fn antialias_edge(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  let base = textureLoad(src_tex, vec2<i32>(i32(x), i32(y))).x;
  let base_a = unpack_a(base);
  if (base_a <= 0.0) {
    textureStore(dst_tex, vec2<i32>(i32(x), i32(y)), vec4<u32>(base, 0u, 0u, 0u));
    return;
  }
  ...
  for (var ky: i32 = -2; ky <= 2; ky = ky + 1) {
    for (var kx: i32 = -2; kx <= 2; kx = kx + 1) {
      ...
      let s = textureLoad(src_tex, vec2<i32>(nx, ny)).x;
      let sa = unpack_a(s);
      if (sa <= 0.0) {
        continue;
      }
  ...
  textureStore(dst_tex, vec2<i32>(i32(x), i32(y)), vec4<u32>(out, 0u, 0u, 0u));
}
