...
impl PsdLayer {
    ...
    /// Return the PSD 4-byte blend mode key (e.g. `b"norm"`, `b"mul "`).
    pub fn blend_mode_key(&self) -> [u8; 4] {
        self.layer_properties.blend_mode.key_bytes()
    }

    /// Get one of the layer channels as decompressed bytes (one byte per pixel).
    ///
    /// For RLE channels, bytes are PackBits-decoded. The returned vector is the raw channel
    /// plane in layer coordinate space (top-left is index 0).
    pub fn channel_bytes(&self, channel: PsdChannelKind) -> Option<Vec<u8>> {
        match self.get_channel(channel)? {
            ChannelBytes::RawData(data) => Some(data.clone()),
            ChannelBytes::RleCompressed(data) => Some(rle_decompress(data)),
        }
    }
...
impl BlendMode {
    ...
    pub(crate) fn key_bytes(&self) -> [u8; 4] {
        match self {
...
fn rle_decompress(bytes: &[u8]) -> Vec<u8> {
    ...
    while i < bytes.len() {
        ...
        if header == -128 {
            ...
            decompressed.extend_from_slice(&bytes[i..end]);
            i = i.saturating_add(count);
        } else {
            if i >= bytes.len() {
                break;
            }
            let repeat = 1i16 - (header as i16);
            let byte = bytes[i];
            i += 1;
