...
class _BedrockModelZBufferViewState extends State<_BedrockModelZBufferView> {
  ...
  static int _hash2d(int x, int y, int seed) {
  ...
  static double _rand2d(int x, int y, int seed) {
    final int h = _hash2d(x, y, seed);
    return (h & 0xffff) / 65535.0;
  }

  static double _smooth01(double t) => t * t * (3.0 - 2.0 * t);
  ...
  static Uint8List _buildCloudMask({
    required int width,
    required int height,
    required int seed,
  }) {
  ...
  static void _compositeSkyboxBackground({
    required Uint8List rgba,
    required int width,
    required int height,
    ...
    final int threshold =
        (coverage.clamp(0.0, 0.92) * 255).round().clamp(0, 254);
    final int denom = 255 - threshold;
    final int cloudStrength = ((0.25 + 0.60 * dayBlend) *
            (skybox.isDark ? 0.65 : 1.0) *
            255)
        .round()
        .clamp(0, 255);
    ...
    for (int y = 0; y < height; y++) {
      ...
      final int baseR =
          (zenithR * (255 - lerpT) + horizonR * lerpT) ~/ 255;
      final int baseG =
          (zenithG * (255 - lerpT) + horizonG * lerpT) ~/ 255;
      final int baseB =
          (zenithB * (255 - lerpT) + horizonB * lerpT) ~/ 255;
